...
################### TOKEN RULES ##################
def t_BEGINTABLE(t):
    r'text:.\'Daily.Deaths\'|name:.\'Daily.Deaths\''
    return t
...
def p_collectdate(p):
    ...
    global date
    if p[1] == "text: 'Daily Deaths'":  # Dates are coming up
...
def p_collectcount(p):
    ...
    if p[1] == "name: 'Daily Deaths'":
        global count
        content = p[2]                  # This contains some unnecessary text, thus cleaning it
        content = content[len('data: ['):-1]
        count = content.split(',')
        count = ['0' if val == 'null' else val for val in count]  # Changing the Null values with 0
        ...
        if continent_folder == 'main_webpage.html':
            ...
            if os.path.isdir(continent_path):
                ...
                for country_file in os.listdir(continent_path):
                    ...
                    # Writing the contents into a file named {country name}_daily_deaths.txt in Stats folder
                    if len(count) != 0:
                        stats_dir = os.path.join(root_dir,'Stats')
                        os.chdir(stats_dir)  # Going to Stats
                        fp = open(country_file[:-5]+'_daily_deaths.txt','w')
