...
################### TOKEN RULES ##################
def t_BEGINTABLE(t):
    r'text:.\'Daily.New.Cases\'|name:.\'Daily.Cases\''
    return t
        ...
        if continent_folder == 'main_webpage.html':
            ...
            if os.path.isdir(continent_path):
                ...
                for country_file in os.listdir(continent_path):
                    ...
                    # Writing the contents into a file named {country name}_daily_cases.txt in Stats folder
                    if len(count) != 0:
                        stats_dir = os.path.join(root_dir,'Stats')
                        os.chdir(stats_dir)  # Going to Stats
                        fp = open(country_file[:-5]+'_daily_cases.txt','w')
